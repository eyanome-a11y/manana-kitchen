...
// KV store (dishes, events, week config)
...
app.get('/api/tasks', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM tasks ORDER BY day_key, created_at');
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/tasks', async (req, res) => {
  try {
    const r = await pool.query('INSERT INTO tasks(day_key,text) VALUES($1,$2) RETURNING *', [req.body.day_key, req.body.text]);
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.patch('/api/tasks/:id', async (req, res) => {
  try {
    const r = await pool.query('UPDATE tasks SET done=$1 WHERE id=$2 RETURNING *', [req.body.done, req.params.id]);
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/stock', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM stock_items ORDER BY expires_at');
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/stock', async (req, res) => {
  try {
    ...
    const r = await pool.query(
      'INSERT INTO stock_items(name,quantity,produced_at,expires_at,notes) VALUES($1,$2,$3,$4,$5) RETURNING *',
...
app.patch('/api/stock/:id', async (req, res) => {
  try {
    const r = await pool.query('UPDATE stock_items SET active=$1 WHERE id=$2 RETURNING *', [req.body.active, req.params.id]);
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/temps', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM temp_logs ORDER BY logged_at DESC LIMIT 30');
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/temps', async (req, res) => {
  try {
    const r = await pool.query(
      'INSERT INTO temp_logs(fridge_temp,freezer_temp,notes) VALUES($1,$2,$3) RETURNING *',
      [req.body.fridge_temp, req.body.freezer_temp, req.body.notes||'']
    );
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Routes - main app and sylvi expense page
